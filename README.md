...
```javascript
// Create a tile map
const tilemap = await game.createTileMap({
    layers: {
...
// When two living things come close to each other, a specific song is heard.
await pixalo.audio.play('music', {
    volume: 0
});
game.audio.setSpatialRelation('music', entity1, entity2, {
    maxDistance: 600,
    curve: v => Math.pow(1 - v, 3)
});
```
...
game.on(['mousemove', 'touchmove'], event => {
    ...
    game.camera.moveBy(-dx, -dy, true).cancelFollow();
